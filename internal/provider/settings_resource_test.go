...
func TestAccSettingsResource_NetworkRetry(t *testing.T) {
	defer gock.OffAll()

	gock.New("https://api.supabase.com").
		Get("/v1/projects/testproject/network-restrictions").
		Reply(http.StatusOK).
		JSON(api.NetworkRestrictionsResponse{
			Config: api.NetworkRestrictionsRequest{
				DbAllowedCidrs:   Ptr([]string{}),
				DbAllowedCidrsV6: Ptr([]string{}),
			},
		})

	// First POST returns 500, second POST succeeds after retry.
	gock.New("https://api.supabase.com").
		Post("/v1/projects/testproject/network-restrictions").
		Reply(http.StatusInternalServerError).
		JSON(map[string]string{
			"message": "error adding pooler tenant for us-east-1/testproject",
		})
	gock.New("https://api.supabase.com").
		Post("/v1/projects/testproject/network-restrictions").
		Reply(http.StatusCreated).
...
`,
				...
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("supabase_settings.test", "project_ref", "testproject"),
					resource.TestCheckResourceAttr("supabase_settings.test", "id", "testproject"),
				),
			},
		},
	})
